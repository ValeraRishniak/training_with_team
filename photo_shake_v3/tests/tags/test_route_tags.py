...
def tag(user, token, session):
    ...
    It then queries the database for the current user and tag.
    If there is no tag it creates one with a title of dog and adds it to the database.
    
    :param user: Get the user id from the database
    :param token: Authenticate the user
    :param session: Query the database
    :return: An object of type tag, which is a sqlalchemy model
    """
...
def test_create_tag(body, client, token):
    """
    The test_create_tag function tests the creation of a new tag.
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        ...
        response = client.post(f'/api/tags/new/', json=body,
...
def test_read_my_tags(client, token):
    """
    The test_read_my_tags function tests the read_my_tags function in the tags.py file.
    It does this by mocking out the redis cache and returning None, which will cause a call to 
    the database for data. It then makes a GET request to /api/tags/my/, with an Authorization header containing 
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        r_mock.get.return_value = None
        response = client.get(f'/api/tags/my/',
                            headers={"Authorization": f"Bearer {token}"})
        assert response.status_code == 200, response.text
        data = response.json()
...
def test_read_all_tags(client, token):
    """
    The test_read_all_tags function tests the read_all_tags function in the tag.py file.
    It does this by mocking out the redis cache and then making a GET request to /api/tags/all/.
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        r_mock.get.return_value = None
        response = client.get(f'/api/tags/all/',
                            headers={"Authorization": f"Bearer {token}"})
        assert response.status_code == 200, response.text
        data = response.json()
        assert isinstance(data, list)
...
def test_read_tag_by_id(tag, client, token):
    """
    The test_read_tag_by_id function tests the read_tag_by_id function in the tag.py file.
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        r_mock.get.return_value = None
        response = client.get(f'/api/tags/by_id/{tag.id}',
                            headers={"Authorization": f"Bearer {token}"})
        assert response.status_code == 200, response.text
        data = response.json()
...
def test_read_tag_by_id_not_found(tag, client, token):
    """
    The test_read_tag_by_id_not_found function tests the read_tag_by_id function in the tag.py file.
    ...
    The response variable stores the result of calling client's get method on /api/tags/by_id/{tag id + 1} with an authorization header containing our token as its value (this will return 404 because we are trying to access a tag that doesn't
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        r_mock.get.return_value = None
        response = client.get(f'/api/tags/by_id/{tag.id+1}',
                            headers={"Authorization": f"Bearer {token}"})
...
def test_update_tag(tag, client, token):
    ...
    the client's put request to /api/tags/upd_tag/{tag.id}. The test asserts that the 
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        r_mock.get.return_value = None
        response = client.put(f'/api/tags/upd_tag/{tag.id}',
...
def test_update_tag_not_found(tag, client, token):
    ...
    Then, we make a PUT request to /api/tags/upd_tag/{id} with an id that doesn't exist in our database and check for 404 status code and &quot;Tag not found&quot; detail message.
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        r_mock.get.return_value = None
        response = client.put(f'/api/tags/upd_tag/{tag.id+1}',
                            json={"title": "new_test_tag"},
                            headers={"Authorization": f"Bearer {token}"})
        assert response.status_code == 404, response.text
        data = response.json()
...
def test_delete(tag, client, token):
    """
    The test_delete function tests the DELETE /api/tags/del/{tag_id} endpoint.
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        r_mock.get.return_value = None
        response = client.delete(f"/api/tags/del/{tag.id}",
            headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 200, response.text
        data = response.json()
...
def test_repeat_delete_tag(tag, client, token):
    """
    The test_repeat_delete_tag function tests the repeat deletion of a tag.
        ...
        r mock's get method returns None when called by the delete method on client which deletes a tag with id 1 from tags table in database using an HTTP DELETE request and headers containing Authorization header set to Bearer followed by token value passed into test case as argument. 
        Assertions are
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        r_mock.get.return_value = None
        response = client.delete(f"/api/tags/del/{tag.id-1}",
            headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 404, response.text
        data = response.json()
