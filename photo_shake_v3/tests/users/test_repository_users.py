...
async def test_get_user_profile(new_user, session):
    ...
    assert response.username == "artur4ik"
    assert response.email == "artur4ik@example.com"
    assert response.foto_count == 0
    assert response.comment_count == 0
    assert response.rates_count == 0
    
    
@pytest.mark.asyncio
async def test_get_all_commented_fotos(new_user, session):
    """
    The test_get_all_commented_fotos function tests the get_all_commented_fotos function in the repository.py file.
    The test ensures that a list is returned and that it has no elements.
    
    :param new_user: Get the user_id of the user
    :param session: Pass the database session to the function
    :return: A list of commented fotos
    """
    response = await repository_users.get_all_commented_fotos(new_user, session)
    assert isinstance(response, list)
    assert len(response) == 0


@pytest.mark.asyncio
async def test_get_all_liked_fotos(new_user, session):
    """
    The test_get_all_liked_fotos function tests the get_all_liked_fotos function in the repository.py file.
        The test checks that a list is returned and that it has no elements.
    
    :param new_user: Pass the user object to the function
    :param session: Create a new session for the database
    :return: A list of all liked fotos for a user
    """
    response = await repository_users.get_all_liked_fotos(new_user, session)
    assert isinstance(response, list)
    assert len(response) == 0
