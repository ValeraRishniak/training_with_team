...
def test_read_user_profile_with_username(client, token):
    ...
    data[&quot;email&quot;] == &quot;artur4ik@example.com&quot;, data[&quot;foto count&quot;] == 0, and data[&quot;
...
def test_read_commented_fotos_by_me_not_found(client, token):
    """
    The test_read_commented_fotos_by_me_not_found function tests the read_commented_fotos_by_me function in the users.py file.
    ...
    with patch.object(auth_service, 'redis_cache') as r_mock:
        ...
        response = client.get(
            "/api/users/commented_fotos_by_me/",
            headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 404, response.text
        data = response.json()
...
def test_read_rated_fotos_by_me_not_found(client, token):
    """
    The test_read_rated_fotos_by_me_not_found function tests the read_rated_fotos_by_me function in the users.py file.
    The test is designed to check that a 404 error code is returned when there are no rated fotos by me.
