...
def test_create_comment(client, token):
    ...
    The function uses the client fixture to make a POST request to the /api/comments/new/&lt;foto_id&gt; endpoint,
...
def test_create_comment_2(client, token):
    ...
    The test_create_comment_2 function tests the creation of a new comment.
        The test_create_comment_2 function is similar to the test_create_comment function, but it uses a different foto id.
        This allows us to check that comments are being created for multiple fotos.
...
def test_comm_foto_auth(client, session, token):
    """
    The test_comm_foto_auth function tests the ability to get comments by author and foto id.
        It does this by making a GET request to the api/comments/foto_by_author endpoint with an 
    ...
    """
    response = client.get(
        "api/comments/foto_by_author/1/1",
        headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200, response.text
