...
from src.database.models import User, Comment, Foto
...
@pytest.mark.asyncio
async def test_create_comment(foto, new_user, session):
    """
    The test_create_comment function tests the create_comment function in repository_comments.py
        ...
        user id, and foto id. The response is then checked to make sure that it has been created correctly.
    ...
    :return: A response with the text &quot;test_comment&quot;, user_id 1 and foto_id 1
    """
    comment = CommentBase(
        text="test_comment",
    )
...
@pytest.mark.asyncio
async def test_show_user_foto_comments(new_user, session):
    """
    The test_show_user_foto_comments function tests the show_user_foto_comments function in the repository.py file.
    The test is successful if it returns a list of comments that belong to a specific user and foto.
    
    :param new_user: Create a new user
    :param session: Pass the database session to the repository function
    :return: A list of comments for a specific user and foto
    """
    response = await repository_comments.show_user_foto_comments(1, 1, session)
    assert isinstance(response, list)
    assert response[0].user_id == 1
