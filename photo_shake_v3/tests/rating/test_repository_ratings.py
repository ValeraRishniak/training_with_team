...
from src.database.models import User, Foto, Rating
...
def foto(session):
    ...
    The foto function creates a new foto in the database.
        Args:
            session (sqlalchemy.orm.session): The current SQLAlchemy session object, which is used to add and commit the new foto to the database.
...
def rating(session):
    ...
    The rating function creates a rating object and adds it to the database.
        ...
        Returns:
            A Rating object with the following attributes: id, rate, created_at, foto_id and user_id.
...
@pytest.mark.asyncio
async def test_create_rate(foto, new_user, session):
    """
    The test_create_rate function tests the create_rate function in repository_ratings.py
        It creates a new user and foto, then uses those to test the create_rate function.
        The response is checked for correct values.
    
    :param foto: Pass the foto object to the function
...
async def test_edit_rate(new_user, session):
    ...
    The test_edit_rate function tests the edit_rate function in repository_ratings.py
        It checks if the rate is 5, user id is 1 and foto id is 1
...
async def test_delete_rate(new_user, session):
    ...
    The test_delete_rate function tests the delete_rate function in repository_ratings.py
        ...
        a session object, and an integer representing the foto to be rated. The response is then 
...
@pytest.mark.asyncio
async def test_user_rate_foto(new_user, session):
    """
    The test_user_rate_foto function tests the user_rate_foto function in repository.py
    ...
    :return: The rate, user_id and foto_id of the new rating
    """
    response = await repository_ratings.user_rate_foto(1, 1, session, new_user)
    assert response.rate == 4
    assert response.user_id == 1
    assert response.foto_id == 1
