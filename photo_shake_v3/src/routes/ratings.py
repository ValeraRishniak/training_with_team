...
from src.schemas import RatingModel, FotoResponse
...
allowed_user_foto_rate = RoleChecker([UserRoleEnum.admin])
allowed_commented_by_user = RoleChecker([UserRoleEnum.admin, UserRoleEnum.moder, UserRoleEnum.user])


@router.post("/fotos/{foto_id}/{rate}", response_model=RatingModel, dependencies=[Depends(allowed_create_ratings)])
async def create_rate(foto_id: int, rate: int = Path(description="From one to five stars of rating.", ge=1, le=5),
                      db: Session = Depends(get_db), current_user: User = Depends(auth_service.get_current_user)):
    """
    The create_rate function creates a new rate for the foto with the given ID. The function takes in an integer
    ...
    :param rate: int: Set the rating of a foto
    ...
    :return: The new rate
    """
    new_rate = await repository_ratings.create_rate(foto_id, rate, db, current_user)
    if new_rate is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=message.NO_FOTO_ID)
    return new_rate
...
@router.get("/user_foto/{user_id}/{foto_id}", response_model=RatingModel,
            dependencies=[Depends(allowed_user_foto_rate)])
async def user_rate_foto(user_id: int, foto_id: int, db: Session = Depends(get_db),
                         current_user: User = Depends(auth_service.get_current_user)):
    """
    The user_rate_foto function allows a user to rate a foto.
        The function takes in the user_id and foto_id as parameters, along with the database session and current user.
        If no rating is found, an HTTPException is raised.

    :param user_id: int: Identify the user who is rating a foto
    :param foto_id: int: Get the foto id
    :param db: Session: Pass the database connection to the function
    :param current_user: User: Get the user_id from the token
    :return: A rating object
    """
    rate = await repository_ratings.user_rate_foto(user_id, foto_id, db, current_user)
    if rate is None:
