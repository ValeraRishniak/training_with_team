...
from src.schemas import FotoResponse, UserProfileModel, UserResponseModel, RequestEmail, UserDb, RequestRole
...
@router.get("/commented_fotos_by_me/", response_model=List[FotoResponse])
async def read_commented_fotos_by_me(db: Session = Depends(get_db),
            current_user: User = Depends(auth_service.get_current_user)):
    """
    The read_commented_fotos_by_me function returns all fotos that the current user has commented on.
    
    :param db: Session: Get the database session
    :param current_user: User: Get the user that is currently logged in
    :return: A list of fotos that the user has commented on
    """
    fotos = await repository_users.get_all_commented_fotos(current_user, db)
    if not fotos:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=NOT_FOUND)
    return fotos


@router.get("/rated_fotos_by_me/", response_model=List[FotoResponse])
async def read_liked_fotos_by_me(db: Session = Depends(get_db),
            current_user: User = Depends(auth_service.get_current_user)):
    """
    The read_liked_fotos_by_me function returns all fotos liked by the current user.
        The function is called when a GET request is made to the /users/me/liked_fotos endpoint.
    
    :param db: Session: Pass the database connection to the function
    :param current_user: User: Get the user object of the current logged in user
    :return: A list of fotos that the user liked
    """
    fotos = await repository_users.get_all_liked_fotos(current_user, db)
    if not fotos:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=NOT_FOUND)
    return fotos
