...
@router.post("/new/{foto_id}", response_model=CommentModel, dependencies=[Depends(allowed_create_comments)])
async def create_comment(foto_id: int, body: CommentBase, db: Session = Depends(get_db),
                         current_user: User = Depends(auth_service.get_current_user)):
    """
    The create_comment function creates a new comment for the foto with the given id.
    ...
    :param foto_id: int: Specify the foto that the comment is being created for
    ...
    """
    new_comment = await repository_comments.create_comment(foto_id, body, db, current_user)
    return new_comment
...
@router.get("/foto_by_author/{user_id}/{foto_id}", response_model=List[CommentModel],
            dependencies=[Depends(allowed_get_comments)])
async def by_user_foto_comments(user_id: int, foto_id: int, db: Session = Depends(get_db),
                                current_user: User = Depends(auth_service.get_current_user)):
    """
    The by_user_foto_comments function returns all comments for a given user and foto.
        Args:
            user_id (int): The id of the user whose comments are being retrieved.
            foto_id (int): The id of the foto whose comments are being retrieved.
        Returns:
            A list containing all comment objects associated with a given user and foto.
    
    :param user_id: int: Specify the user_id of the user whose comments we want to retrieve
    :param foto_id: int: Get the comments for a specific foto
    :param db: Session: Access the database
    :param current_user: User: Get the current user who is logged in
    :return: A list of comments that belong to a foto
    """
    comments = await repository_comments.show_user_foto_comments(user_id, foto_id, db)
    if comments is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=message.COMM_NOT_FOUND)
    return comments
