...
allowed_get_all_tags = RoleChecker([UserRoleEnum.admin])
allowed_remove_tag = RoleChecker([UserRoleEnum.admin])
allowed_edit_tag = RoleChecker([UserRoleEnum.admin])
...
async def create_tag(body: TagBase, db: Session = Depends(get_db),
    ...
    The create_tag function creates a new tag in the database.
        ...
        If it succeeds, then we create a new Tag object and add it to our database session (db). 
            ...
            Args:
                body (TagBase): A TagBase object containing information about our tag that we want to create in our database. This is validated by pydantic before being passed into this function. 
                db (
    
    :param body: TagBase: Define the type of data that will be passed to the function
...
@router.put("/upd_tag/{tag_id}", response_model=TagResponse, dependencies=[Depends(allowed_edit_tag)])
async def update_tag(body: TagBase, tag_id: int, db: Session = Depends(get_db),
            current_user: User = Depends(auth_service.get_current_user)):
    """
    ...
    :param body: TagBase: Pass the data from the request body to the function
...
@router.delete("/del/{tag_id}", response_model=TagResponse, dependencies=[Depends(allowed_remove_tag)])
