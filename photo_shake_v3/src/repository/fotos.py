...
async def create_foto(request: Request, title: str, descr: str, tags: List, file: UploadFile, db: Session, current_user: User) -> Foto:
    """
    ...
    public_id=f'PhotoShake/{current_user.id}'
    ...
    url = cloudinary.CloudinaryImage(f'PhotoShake/{current_user.id}')\
...
async def get_my_fotos(skip: int, limit: int, user: User, db: Session) -> List[Foto]:
    ...
    """
    return db.query(Foto).filter(Foto.user_id == user.id).offset(skip).limit(limit).all()
...
async def get_foto_by_id(foto_id: int, user: User, db: Session) -> Foto:
    ...
    foto = db.query(Foto).filter(
        and_(Foto.user_id == user.id, Foto.id == foto_id)).first()
    return foto
...
async def get_fotos_by_title(foto_title: str, user: User, db: Session) -> List[Foto]:
    ...
    """
    return db.query(Foto).filter(func.lower(Foto.title).like(f'%{foto_title.lower()}%')).all()
...
async def get_foto_by_keyword(keyword: str, db: Session):
    ...
    :return: A list of foto objects
    """
    return  db.query(Foto).filter(or_(
        func.lower(Foto.title).like(f'%{keyword.lower()}%'),
        func.lower(Foto.descr).like(f'%{keyword.lower()}%')
        )).all()

    
async def update_foto(foto_id: int, body: FotoUpdate, user: User, db: Session) -> Foto | None:
    """
