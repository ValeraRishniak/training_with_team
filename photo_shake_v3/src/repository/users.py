...
from src.database.models import User, UserRoleEnum, Comment, Rating, Foto, BlacklistToken
...
async def edit_my_profile(file, new_username, user: User, db: Session) -> User:
    ...
    init_cloudinary()
    cloudinary.uploader.upload(file.file, public_id=f'PhotoShake/{me.username}',
                               overwrite=True, invalidate=True)
    url = cloudinary.CloudinaryImage(f'PhotoShake/{me.username}')\
...
async def get_user_profile(username: str, db: Session) -> User:
    ...
    if user:
        foto_count = db.query(Foto).filter(Foto.user_id == user.id).count()
...
async def get_all_commented_fotos(user: User, db: Session):
    """
    The get_all_commented_fotos function returns all fotos that a user has commented on.
    
    :param user: User: Get the user object from the database
    :param db: Session: Pass the database session to the function
    :return: All fotos that have been commented on by a user
    """
    return db.query(Foto).join(Comment).filter(Comment.user_id == user.id).all()


async def get_all_liked_fotos(user: User, db: Session):
    """
    The get_all_liked_fotos function returns all fotos that a user has liked.
        Args:
            user (User): The User object to get the liked fotos for.
            db (Session): A database session to use for querying the database.
        Returns:
            List[Foto]: A list of Foto objects that have been liked by the specified User.
    
    :param user: User: Get the user's id
    :param db: Session: Pass the database session to the function
    :return: A list of fotos that the user liked
    """
    return db.query(Foto).join(Rating).filter(Rating.user_id == user.id).all()
