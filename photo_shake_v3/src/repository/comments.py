...
async def create_comment(foto_id: int, body: CommentBase, db: Session, user: User) -> Comment:

    """
    The create_comment function creates a new comment in the database.
        Args:
            foto_id (int): The id of the foto to which this comment belongs.
    ...
    :param foto_id: int: Identify the foto that the comment is being added to
    ...
    """
    new_comment = Comment(text=body.text, foto_id=foto_id, user_id=user.id)
    db.add(new_comment)
    db.commit()
...
async def delete_comment(comment_id: int, db: Session, user: User) -> None:
    ...
    The delete_comment function deletes a comment from the database.
        Args:
            ...
            user (User): The user who is deleting this foto.
...
async def show_user_foto_comments(user_id: int, foto_id: int, db: Session) -> List[Comment] | None:

    """
    The show_user_foto_comments function returns a list of comments for a given user and foto.
        Args:
    ...
    """
    return db.query(Comment).filter(and_(Comment.foto_id == foto_id, Comment.user_id == user_id)).all()
